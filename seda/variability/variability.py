...
def classify_variability(
    ...
    *,
    normalize: bool = True,
    scheme: str = "Oliveros-Gomez+(2022-2024)",
) -> VariabilityResult:
    """
    High-level wrapper to classify brown dwarf variability.
    ...
    spectral_type : {"L", "T"}
        Coarse spectral type of the object.
    normalize : bool, default True
        If True, the flux is median-normalized before computing indices.
    scheme : str, default "Oliveros-Gomez+2022/2024"
        Name of the classification scheme. Reserved for future schemes.
    ...
    result : VariabilityResult
        ...
        - scheme : str  
            Name or reference of the variability scheme (e.g., "Oliveros-Gomez+2022",
            "Oliveros-Gomez+2024").
    
        - is_candidate_variable : bool  
            Final classification flag. True if the number of triggered index–index
            regions meets or exceeds the adopted threshold.
        ...
        - threshold : int  
            Minimum number of triggered regions required to be classified as a
            candidate variable.
    
        - indices : dict[str, float]  
            Dictionary of computed NIR spectral indices. Keys correspond to the
            physical index names (e.g., "J", "H", "Jslope", "Jcurve", "H2OJ", "CH4J"),
            and values are the numerical index values.
    
        - regions_triggered : list[str]  
            List of region identifiers (or names) for which the target falls inside
            the corresponding variability polygon.
    
        - normalize : bool  
            Whether a median flux normalization was applied to the input spectrum
            prior to computing the indices.
    ...
    spt = spectral_type.strip().upper()
    if spt.startswith("T"):
        return classify_T_variability(
            wavelength, flux, normalize=normalize, scheme=scheme
        )
    elif spt.startswith("L"):
        return classify_L_variability(
            wavelength, flux, normalize=normalize, scheme=scheme
        )
    else:
        raise ValueError(
