...
def generate_model_spectrum(Teff, logg, logKzz, Z, CtoO, grid=None, model=None, model_dir=None, Teff_range=None, logg_range=None, save_spectrum=False):
	...
	if grid is None:
		...
		if Teff_range is None: print('ERROR: missing Teff range to read a model grid subset'), exit()
		if logg_range is None: print('ERROR: missing logg range to read a model grid subset'), exit()
		if Teff<Teff_range.min() or Teff>Teff_range.max(): print('ERROR: Teff value is out of the Teff range'), exit() # show a warning if a Teff value is out of the loaded model grid.
		if logg<logg_range.min() or logg>logg_range.max(): print('ERROR: logg value is out of the logg range'), exit() # show a warning if a logg value is out of the loaded model grid.
		out_grid_ranges = grid_ranges(model) # read parameters' ranges
		if logKzz<out_grid_ranges['logKzz'].min() or logKzz>out_grid_ranges['logKzz'].max(): print('ERROR: logKzz value is out of the grid coverage'), exit()
		if Z<out_grid_ranges['Z'].min() or Z>out_grid_ranges['Z'].max(): print('ERROR: Z value is out of the grid coverage'), exit()
		if CtoO<out_grid_ranges['CtoO'].min() or CtoO>out_grid_ranges['CtoO'].max(): print('ERROR: C/O value is out of the grid coverage'), exit()

		grid = read_grid(model, model_dir, Teff_range, logg_range)
...
#	'''
	...
	syn_spectrum = generate_model_spectrum(Teff_interpol=Teff_med, logg_interpol=logg_med, 
